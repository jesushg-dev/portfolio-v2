...
const PortfolioItem: FC<IPortfolioItemProps> = ({
  ...
  return (
    <div className="group relative h-60 max-w-sm overflow-hidden rounded bg-background-50 shadow-lg">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-background-100 p-4 text-secondaryText-700">
        <p className="mb-1 text-sm font-semibold">{title}</p>
        <p className="text-xs text-secondaryText-500">{description}</p>
      </div>
      <div className="absolute inset-0 isolate z-20 flex flex-col bg-black bg-opacity-80 opacity-0 transition-opacity group-hover:opacity-100">
        <div className="flex grow flex-col items-center justify-center gap-4 px-4 ">
          <a
            ...
            <span className="flex items-center justify-center gap-2 text-xs text-neutralText-50">
        ...
        <div className="flex w-full gap-4 overflow-x-auto bg-background-100 py-5 px-4 text-secondaryText-700">
          {made_with?.map((tech) => (
      ...
      {is_private && (
        <span
          ...
          className="whitespace-no-wrap absolute top-0 right-0 z-30 origin-bottom-left -translate-y-full translate-x-1/3 rotate-45 transform bg-primary-600 px-5 py-1 text-center text-xs uppercase tracking-wider text-neutralText-50 shadow-lg">
          {privateName}
        </span>
      )}
    </div>
