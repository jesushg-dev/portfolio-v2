...
export function fillMissingTranslations<T extends object>(
  ...
  if (translationKey && rawData && Array.isArray(rawData[translationKey])) {
    const rawTranslations = rawData[translationKey] as Array<Record<string, unknown>>;
  ...
  else if (rawData?.title && typeof rawData.title === "object" && rawData.title !== null) {
      ...
      Object.entries(titleObj.translations).forEach(([langId, titleStr]) => {
        if (titleStr) {
          map.set(langId, {
            ...
            title: titleStr,
            description: descObj.translations?.[langId as keyof typeof descObj.translations] ?? "",
          });
        }
      });
    }
