...
interface DroppableContainerData {
  type?: string;
...
const coordinateGetter: KeyboardCoordinateGetter = (event, { context }) => {
  ...
  if (directions.includes(event.code)) {
    ...
    for (const entry of droppableContainers.getEnabled()) {
      ...
      const data = entry.data.current as DroppableContainerData | undefined;
    ...
    <KanbanColumnContext.Provider value={columnContext}>
      ...
      >
        <ColumnPrimitive
          ...
          style={composedStyle}
          className={cn(
            "bg-muted/40 border-border flex size-full flex-col gap-2 rounded-lg border p-2.5 aria-disabled:pointer-events-none aria-disabled:opacity-50",
            {
