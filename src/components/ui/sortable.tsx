...
import { type ComponentProps, createContext, useCallback, useContext, useId, useMemo, useState, type ReactNode, type CSSProperties } from "react";
...
function Sortable<T>(props: SortableProps<T>) {
  ...
  const announcements: Announcements = useMemo(() => {
    const getSortableIndex = (item: { data?: { current?: unknown } }) =>
      (item?.data?.current as { sortable?: { index: number } } | undefined)?.sortable?.index ?? 0;

    return {
      onDragStart({ active }) {
        const activeValue = active.id.toString();
        return `Grabbed sortable item "${activeValue}". Current position is ${getSortableIndex(active) + 1} of ${value.length}. Use arrow keys to move, space to drop.`;
      },
      onDragOver({ active, over }) {
        if (over) {
          const overIndex = getSortableIndex(over);
          const activeIndex = getSortableIndex(active);
...
interface SortableContentProps extends ComponentProps<"div"> {
  strategy?: SortableContextProps["strategy"];
  children: ReactNode;
  asChild?: boolean;
  withoutSlot?: boolean;
...
interface SortableItemHandleProps extends ComponentProps<"button"> {
  asChild?: boolean;
}
