...
const Terminal: FC<ITerminalProps> = ({ code }) => {
  ...
  return (
    <div
      className={`text-neutralText-50 bg-background-900 flex w-full flex-col overflow-hidden rounded-lg shadow-2xl theme-main-${
        isDark ? "dark" : "light"
      }`}
    >
      ...
      <pre className="-ml-6 flex grow">
        <code className="hljs grow">{code}</code>
      </pre>
    </div>
  );
};
