...
function makeMockColumn(initialValue: unknown = undefined) {
  ...
  return {
    ...
    getFacetedUniqueValues: () => new Map([["opt1", 5], ["opt2", 10]]),
    columnDef: {
      meta: {
        unit: "USD",
        defaultRange: [0, 50],
...
describe("DataTableFacetedFilter", () => {
  it("renders trigger button with title and options count", () => {
    ...
    render(<DataTableFacetedFilter column={col} title="Status" options={options} multiple={true} />);
  ...
  it("handles single selection and clearing filter", () => {
    ...
    render(<DataTableFacetedFilter column={col} title="Single Filter" options={options} multiple={false} />);
  ...
  it("renders faceted filter without selected values", () => {
    ...
    render(<DataTableFacetedFilter column={col} title="Category" options={options} multiple={true} />);
...
describe("DataTableDateFilter", () => {
  it("renders date filter button with single date", () => {
    ...
    render(<DataTableDateFilter column={col} title="Created Date" multiple={false} />);
  ...
  it("renders range date filter button when multiple is true", () => {
    ...
    render(<DataTableDateFilter column={col} title="Date Range" multiple={true} />);
