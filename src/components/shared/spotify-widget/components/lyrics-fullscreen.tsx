...
import { buildSpotifyFullscreenBg } from "../hooks/use-album-color";
import { findActiveLyricIndex, parseLrc } from "../utils/parse-lrc";
...
const LyricsFullscreen: FC<LyricsFullscreenProps> = ({
  ...
  const { scrollToElement } = useIOSDevice();
  ...
  useEffect(() => {
    ...
    scrollToElement(activeLineRef.current, {
      block: "center",
      behavior: jumpedToStart ? "instant" : "smooth",
    });
  }, [activeIndex, useSynced, scrollToElement]);
  ...
  return (
    ...
    >
      <header className="relative z-20 shrink-0 px-3.5 pt-9.5 pb-2">
