import { getFilterOperators, getDefaultFilterOperator, getValidFilters, getColumnPinningStyle } from "./data-table";
...
function makeMockColumn(opts: {
  ...
  return {
    ...
    getIsLastColumn: (side: string) => side === "left" && (opts.isLastLeft ?? false),
    getIsFirstColumn: (side: string) => side === "right" && (opts.isFirstRight ?? false),
...
describe("getColumnPinningStyle", () => {
  ...
  it("returns right pinned style with border shadow on first right column", () => {
    const col = makeMockColumn({ pinned: "right", isFirstRight: true, after: 0 });
    const style = getColumnPinningStyle({ column: col, withBorder: true });
...
describe("getValidFilters", () => {
  it("removes filters with empty string values", () => {
    const result = getValidFilters([
      { id: "name", value: "", operator: "iLike", filterId: "f1", rowId: "r1", type: "text", field: "name", variant: "text" },
    ]);
  ...
  it("keeps filters with non-empty string values", () => {
    const result = getValidFilters([
      { id: "name", value: "Alice", operator: "iLike", filterId: "f1", rowId: "r1", type: "text", field: "name", variant: "text" },
    ]);
  ...
  it("keeps filters with isEmpty operator regardless of value", () => {
    const result = getValidFilters([
      { id: "name", value: "", operator: "isEmpty", filterId: "f1", rowId: "r1", type: "text", field: "name", variant: "text" },
    ]);
    expect(result).toHaveLength(1);
  });
  ...
  it("keeps filters with isNotEmpty operator regardless of value", () => {
    const result = getValidFilters([
      { id: "name", value: "", operator: "isNotEmpty", filterId: "f1", rowId: "r1", type: "text", field: "name", variant: "text" },
    ]);
    expect(result).toHaveLength(1);
  });
  ...
  it("keeps filters with non-empty array values", () => {
    const result = getValidFilters([
      { id: "tags", value: ["a", "b"], operator: "inArray", filterId: "f1", rowId: "r1", type: "text", field: "tags", variant: "multiSelect" },
    ]);
    expect(result).toHaveLength(1);
  });
  ...
  it("removes filters with empty array values", () => {
    const result = getValidFilters([
      { id: "tags", value: [], operator: "inArray", filterId: "f1", rowId: "r1", type: "text", field: "tags", variant: "multiSelect" },
    ]);
    expect(result).toHaveLength(0);
  });
  ...
  it("removes filters with null values", () => {
    const result = getValidFilters([
      { id: "date", value: null, operator: "eq", filterId: "f1", rowId: "r1", type: "text", field: "date", variant: "date" },
    ]);
    expect(result).toHaveLength(0);
  });
  ...
  it("removes filters with undefined values", () => {
    const result = getValidFilters([
      { id: "date", value: undefined, operator: "eq", filterId: "f1", rowId: "r1", type: "text", field: "date", variant: "date" },
    ]);
    expect(result).toHaveLength(0);
  });
  ...
  it("returns all filters when all are valid", () => {
    const result = getValidFilters([
      { id: "a", value: "x", operator: "iLike", filterId: "f1", rowId: "r1", type: "text", field: "a", variant: "text" },
      { id: "b", value: "isEmpty", operator: "isEmpty", filterId: "f2", rowId: "r2", type: "text", field: "b", variant: "text" },
    ]);
