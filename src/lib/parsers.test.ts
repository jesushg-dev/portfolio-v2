...
describe("parsers", () => {
  describe("getSortingStateParser", () => {
    ...
    it("handles validKeys set or array filter", () => {
      ...
      expect(parserArray.parse(JSON.stringify([{ id: "title", desc: false }]))).toEqual([{ id: "title", desc: false }]);
      expect(parserArray.parse(JSON.stringify([{ id: "unknown", desc: false }]))).toBeNull();

      const parserSet = getSortingStateParser(new Set(["title"]));
      expect(parserSet.parse(JSON.stringify([{ id: "title", desc: false }]))).toEqual([{ id: "title", desc: false }]);
      expect(parserSet.parse(JSON.stringify([{ id: "other", desc: false }]))).toBeNull();
    });
  ...
  describe("getFiltersStateParser", () => {
    ...
    it("compares equality with eq method", () => {
      const parser = getFiltersStateParser();
      const f1 = [
        { id: "title", value: "a", variant: "text" as const, operator: "iLike" as const, filterId: "f1" },
      ];
      const f2 = [
        { id: "title", value: "a", variant: "text" as const, operator: "iLike" as const, filterId: "f1" },
      ];
      const f3 = [
        { id: "title", value: "b", variant: "text" as const, operator: "iLike" as const, filterId: "f1" },
      ];
