...
describe("getTranslationCompleteness", () => {
  ...
  it("treats whitespace-only strings as empty", () => {
    expect(
      getTranslationCompleteness({ title: "   ", description: "" }, ["title", "description"]),
    ).toBe("empty");
  });

  it("treats non-string values as empty", () => {
    expect(
      getTranslationCompleteness({ title: 123 as unknown as string, description: "" }, ["title", "description"]),
    ).toBe("empty");
  });
});
...
describe("buildStatusByLangIdFromMap", () => {
  ...
  it("returns empty for all languages when map is undefined", () => {
    expect(
      buildStatusByLangIdFromMap(languages, undefined, ["title", "description"]),
...
describe("primaryTranslationMapRequired", () => {
  ...
  it("does nothing when primaryLangId is undefined", () => {
    const ctx = makeCtx();
    const refine = primaryTranslationMapRequired(undefined, "title", "Required");
    refine({ "lang-en": { title: "Hello" } }, ctx as unknown as import("zod").RefinementCtx);
    expect(ctx.issues).toHaveLength(0);
  });

  it("adds an issue when the field is empty", () => {
    const ctx = makeCtx();
    const refine = primaryTranslationMapRequired("lang-en", "title", "Title is required");
    refine({ "lang-en": { title: "" } }, ctx as unknown as import("zod").RefinementCtx);
  ...
  it("does NOT add an issue when the field has content", () => {
    const ctx = makeCtx();
    const refine = primaryTranslationMapRequired("lang-en", "title", "Required");
    refine({ "lang-en": { title: "Hello" } }, ctx as unknown as import("zod").RefinementCtx);
    expect(ctx.issues).toHaveLength(0);
  });
  ...
  it("adds an issue when the primary lang entry is missing entirely", () => {
    const ctx = makeCtx();
    const refine = primaryTranslationMapRequired("lang-en", "title", "Required");
    refine({ "lang-es": { title: "Hola" } }, ctx as unknown as import("zod").RefinementCtx);
    expect(ctx.issues).toHaveLength(1);
  });
});
...
describe("titleDescriptionTranslationMapSchema", () => {
  it("validates a complete map correctly", () => {
    const schema = titleDescriptionTranslationMapSchema("lang-en", "Title required");
  ...
  it("passes when primaryLangId is undefined (no primary validation)", () => {
    const schema = titleDescriptionTranslationMapSchema(undefined, "Title required");
    const result = schema.safeParse({
      "lang-en": { title: "", description: "" },
    });
    expect(result.success).toBe(true);
