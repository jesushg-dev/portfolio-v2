...
export const SkillForm: FC<SkillFormProps> = ({ initialData, languages }) => {
  ...
  const defaultTranslations = isEditMode
    ? (() => {
        const existingTranslations = initialData.SkillTranslation?.map((st) => ({
    ...
    startTransition(async () => {
      try {
        if (isEditMode && values.id) {
          ...
          await Promise.all(
            values.translations.filter(t => t.description.trim() !== "" || t.urlWiki.trim() !== "").map((trans) =>
          ...
          await createSkill.mutateAsync({
            ...
            image: values.image,
            type: values.type,
            translations: values.translations.filter(t => t.description.trim() !== "" || t.urlWiki.trim() !== "").map((t) => ({
