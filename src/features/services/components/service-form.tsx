...
export const ServiceForm: FC<ServiceFormProps> = ({ initialData, languages }) => {
  ...
  const defaultTranslations = fillMissingTranslations(isEditMode ? initialData : {}, languages);
  ...
  const form = useForm<TServiceForm>({
    ...
    defaultValues: {
      ...
      skillIds: isEditMode ? (initialData.ServiceSkill?.map((ss) => ss.skillId) ?? []) : [],
    ...
    startTransition(async () => {
      try {
        if (isEditMode && values.id) {
          ...
          await Promise.all(
            values.translations.filter(t => t.title.trim() !== "" || t.description.trim() !== "").map((trans) =>
    ...
    <Form {...form}>
      <FormRoot onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <FormActions
          ...
          title={t("save")}
        >
          <Button type="button" variant="ghost" onClick={() => router.back()}>{t("cancel") || "Cancel"}</Button>
        </FormActions>
      </FormRoot>
    </Form>
  );
