...
const projectFormSchema = z.object({
  ...
  translations: z
    ...
    .refine((val) => val.length >= 1, { message: "At least one language is required" }),
});
...
export const ProjectForm: FC<ProjectFormProps> = ({ initialData, languages }) => {
  ...
  const defaultTranslations = isEditMode
    ? (() => {
        const existingTranslations = initialData.ProjectTranslation?.map((pt) => ({
          appLanguageId: pt.appLanguageId,
          title: pt.title,
          description: pt.description,
        })) ?? [];

        const existingLangIds = new Set(existingTranslations.map((t) => t.appLanguageId));
  ...
  const form = useForm<TProjectForm>({
    ...
    defaultValues: (isEditMode ? {
        ...
        skillIds:
          initialData.ProjectSkill?.map((ps) => ps.skillId) ?? [],
  ...
  const [activeLangId, setActiveLangId] = useState<string>(
    isEditMode ? (fields[0]?.appLanguageId ?? languages[0]?.id ?? "") : (primaryLang?.id ?? languages[0]?.id ?? "")
  );

  const activeIndex = fields.findIndex((f) => f.appLanguageId === activeLangId);
  const activeLang = languages.find((l) => l.id === activeLangId);

  const onSubmit = useCallback((values: TProjectForm) => {
  ...
  }, [isEditMode, updateProject, syncSkills, upsertTranslation, createProject, utils, router]);
  ...
  return (
    <Form {...form}>
      <FormRoot onSubmit={form.handleSubmit(onSubmit)}>
        ...
        >
          <Button type="button" variant="ghost" onClick={() => router.back()}>{tCommon("cancel") || "Cancel"}</Button>
        </FormActions>
      </FormRoot>
    </Form>
  );
